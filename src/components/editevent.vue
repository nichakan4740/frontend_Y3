...
onBeforeMount(async () => {
  ...
  const file = await fetch(`${import.meta.env.VITE_BASE_URL}api/files/` + params.editid,{
    method: "GET",
    mode: "cors",
    // headers: {
    //   'Authorization' : "Bearer " + localStorage.getItem('accesstoken'),
...
const onFileChange = (e) => {
  filewithid.value = e.target.files[0];
      ...
      <div class="modal-container">
        ...
        <div class="modal-body">
          ...
          <div class="bg-light p-3 rounded">
            ...
            <a class="file" v-for="file in filewithid"> {{ file }} </a>
          </div>
        </div>
        ...
        <div class="modal-footer">
          <div
            class="editbooking"
            @click="saveevent(editwithid.eventNotes, StartTimeISO)"
          >
            Save
          </div>
        </div>
