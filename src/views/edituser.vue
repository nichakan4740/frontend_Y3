...
const userswithid = ref([]);

onBeforeMount(async () => {
  const res = await fetch(`${import.meta.env.VITE_BASE_URL}api/users/` + params.usersid,{
    method: "GET",
    mode: "cors",
    headers: {
  ...
  if (res.status === 200) {
    userswithid.value = await res.json();
    console.log(userswithid.value);
  } 
  if (res.status === 401) {
  //   await fetch(`${import.meta.env.VITE_BASE_URL}api/refresh`,{
  //   method: "GET",
  //   mode: "cors",
  //   headers: {
...
const saveusers = async (name, email, role) => {
  if (confirm("You want to save change") == true) {
    const res = await fetch(
      `${import.meta.env.VITE_BASE_URL}api/users/` + params.usersid,
      {
        method: "PUT",
        headers: {
...
const close = () => appRouter.push({ name: "showusers" });
</script>
    ...
    <div class="modal-wrapper">
      <div class="modal-container">
        <div class="modal-header border-bottom-0">
          <h2 class="modal-title"><b>Edit User</b></h2>
          <button
            type="button"
            class="btn-close"
        ...
        <div class="modal-body">
          ...
          <!-- <div class="bg-light p-3 rounded">
            <p class="text">
              ...
              <input
                ...
                v-model.trim="userswithid.email"
              />
          </div>
          <br /> -->
            ...
            <form action="#" id="form">
              <div class="input-box">
                <input
                  ...
                  v-model.trim="userswithid.email"
                />
                <p v-show="userswithid.email < 1" id="checkname">
          ...
          <div class="bg-light p-3 rounded">
            <p class="text">Role</p>
            <select class="form-select" v-model="userswithid.role">
        ...
        <div class="modal-footer">
          <div
            class="editbooking"
            @click="
              saveusers(userswithid.name, userswithid.email, userswithid.role)
            "
          >
            Save
          </div>
